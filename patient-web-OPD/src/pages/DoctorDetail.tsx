...
export const DoctorDetail: React.FC = () => {
  ...
  const stateDoctor = (location.state as { doctor?: Doctor } | null)?.doctor;

  const { data: fetchedDoctor, isLoading: isDoctorLoading } = useQuery({
    queryKey: ['doctor', id],
    queryFn: () => api.doctorByIdOrSlug(id!),
    enabled: !stateDoctor && !!id,
  });

  const doctor = stateDoctor || fetchedDoctor;

  const dates = useMemo(() => {
    const today = new Date();
    return Array.from({ length: AppConfig.bookingWindowDays + 1 }, (_, i) =>
      addDays(today, i)
    );
  }, []);
  ...
  if (isDoctorLoading && !doctor) {
    return (
      <div>
        <TopBar onBack={() => navigate(-1)} title="Select Slot & Book" />
        <StateView loading />
      </div>
    );
  }
  ...
  if (!doctor) {
    return (
      <div>
        <TopBar onBack={() => navigate('/')} title="Select Slot & Book" />
        <StateView empty="Doctor not found." />
      </div>
    );
  }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <div className="section-card">
            ...
            <div className="date-strip">
              {dates.map((d) => {
                const isSelected =
                  format(d, 'yyyy-MM-dd') === formattedSelectedDate;
                return (
                  <div
                    key={d.toISOString()}
...
const SlotChip: React.FC<{ slot: Slot; onSelect: () => void }> = ({
  slot,
  onSelect,
}) => {
...
const TopBar: React.FC<{ onBack: () => void; title: string }> = ({
  onBack,
  title,
}) => (
